"""Area, centroid, and connected-component metrics for plume image stacks."""
...
class PlumeMetrics:
    ...
    ``PlumeMetrics(time_interval, start_position, position_range)`` and the
    older Plume-Learn style ``PlumeMetrics(start_position, position_range)``.
    """
    ...
    def __init__(
        ...
        *args,
        ...
        if len(args) == 3:
            ...
            raise TypeError(
                "PlumeMetrics expects either (time_interval, start_position, "
    ...
    def calculate_area_for_plumes(self, plumes, return_format='numpy'):
        ...
        plume_indexes = range(plumes.shape[0])
        if self.progress_bar:
            plume_indexes = tqdm(plume_indexes)
        for i in plume_indexes:
