"""X-ray diffraction plotting helpers used alongside plume experiments."""
...
def plot_xrd(ax, files, labels, title=None, xrange=(0,90), diff=1e3, pad_sequence=[], save_file=None):
    """
    ...
    # # pad the shorter scan range
    # X = np.arange(np.max(length_list))
    # for i, Y in enumerate(Ys):
    #     Ys[i] = np.pad(Y, (0, np.max(length_list)-len(Y)), 'constant', constant_values=0)

    colors = cm.viridis(np.linspace(0.2, 0.8, len(Ys)))    
